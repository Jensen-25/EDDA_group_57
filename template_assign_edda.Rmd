...
\]
predictions were computed for an average person (age = 39.2 years, height = 171 cm).

The highest predicted weight loss is observed for **Diet 3 and men (gender = 0)**, with an expected loss of **5.76 kg** (95% CI: [4.47, 7.05]). Therefore, according to the fitted model, men following Diet 3 are predicted to lose the most weight.

```{r, exercise2e_predictions}
# Fit the chosen model (same as 2D)
data2 <- na.omit(data)
...
# Predictions for all diet x gender groups
newdata <- expand.grid(
...
pred <- predict(model_int, newdata = newdata, interval = "confidence")
