...
\hypersetup{
  ...
  pdfauthor={James Bond, group 007},
  hidelinks,
...
source(exc2A.R) \#\# Short introduction to R Markdown
...
\begin{center}\includegraphics{template_assign_edda_files/figure-latex/unnamed-chunk-3-1} \end{center}
...
\FunctionTok{sample}\NormalTok{(x)}
\DocumentationTok{\#\#  [1] "B" "A" "A" "A" "A" "B" "B" "A" "B" "B"}
\NormalTok{x}\OtherTok{=}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  x
## t = 3.5287, df = 99, p-value = 0.000635
...
The value of t-statistics in the above evaluation is -2.52 and the
p-value is 0.0136.
...
influence the weight loss among a population with \texttt{N=78}. Each
person followed a specific diet during 6 weeks and the weight before and
after this time period are measured. The considered dataset is
...
\begin{verbatim}
##   person gender age height preweight diet weight6weeks weight.loss
## 2      1      0  22    159        58    1         54.2         3.8
## 3      2      0  46    192        60    1           54         6.0
## 4      3      0  55    170        64    1         63.3         0.7
## 5      4      0  33    171        64    1         61.1         2.9
## 6      5      0  50    170        65    1         62.2         2.8
## 7      6      0  50    201        66    1           64         2.0
\end{verbatim}

\hypertarget{exercise-2.a}{%
\subsection{exercise 2.A)}\label{exercise-2.a}}

This part contains the informative graphical summary of the effect of
diet on weight loss. Since the data is measured for the same individuals
before and after six weeks on the diet, the data are paired. The
following assumptions have to be met before performing a paired t-test:
independency of pairs, normality of weight loss, and the absence of
extreme outliers.
...
reasonable that the data is normally distributed. \textbf{c)} A boxplot
is created to visually check whether the data contains outliers or not.
The boxplot shows the median, interquartile range, and potential extreme
values. To asses the amount of outliers numerically, the command
\texttt{boxplot.stats(weight\_loss)\$out} is used, which gave
\texttt{numeric(0)} as output, meaning that the data does not contain
any outliers.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Q{-}Q plot for weight loss}
\FunctionTok{qqnorm}\NormalTok{(weight\_loss,}
       \AttributeTok{main =} \StringTok{"Normal Q{-}Q Plot of Weight Loss"}\NormalTok{)}
\FunctionTok{qqline}\NormalTok{(weight\_loss, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{template_assign_edda_files/figure-latex/qqplot-weightloss-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boxplot}\NormalTok{(weight\_loss,}
        \AttributeTok{main =} \StringTok{"Boxplot of Weight Loss"}\NormalTok{,}
        \AttributeTok{ylab =} \StringTok{"Weight lost (kg)"}\NormalTok{,}
        \AttributeTok{col =} \StringTok{"lightblue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{template_assign_edda_files/figure-latex/boxplot-weightloss-1.pdf}
